...
export const ACQUISITION_TAX = {
  ...
  multiHomeRates: {
    ...
    // 비조정대상지역
    //  - 2025.10.15 대책 이후 비조정지역 2주택자도 8%로 강화 (조정지역과 동일).
    //  - 향후 일부 비조정지역에서 1~3% 기본세율 환원 논의 있음 (2026년 미반영).
    unregulated: {
      2: 0.08,
      3: 0.12,
    },
...
export const CAPITAL_GAINS_TAX = {
  ...
  // 다주택자 중과세율 가산
  //  - 2022.5.10 이후 한시 유예가 수 차례 연장되어 왔음.
  //  - 현재 코드는 2026.5.10 시행을 가정하나, 정부 발표에 따라 다시 유예될 수 있음.
  //  - 시행일 변경 시 enforcementDate만 수정하면 전체 계산에 반영됨.
  multiHomeAdditional: {
    twoHomes: 0.20, // 2주택자 +20%p (조정대상지역)
    threeHomes: 0.30, // 3주택 이상 +30%p (조정대상지역)
    enforcementDate: '2026-05-10', // 시행 예정일 (재유예 가능성 있음)
    enforcementNote: '한시 유예 종료 예정. 국세청 공식 발표로 최종 확인 필요.',
  },
...
export const REGULATED_AREAS = {
  ...
  source: '국토교통부 주거정책심의위원회 공고',
  sourceUrl: 'https://www.molit.go.kr',
  note: '조정대상지역은 주거정책심의위 심의 후 수시 변경되므로 매수/매도 전 공식 사이트로 재확인 권장.',
};
